  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "1.5rem 1rem" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input
          style={{ flex: 1, padding: "10px 12px", fontSize: 14, border: "1px solid #ddd", borderRadius: 10, boxSizing: "border-box" }}
          placeholder="新增事項..."
          value={newTitle}
          onChange={e => setNewTitle(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && addTodo()}
        />
        <button onClick={addTodo} disabled={!newTitle.trim()} style={{ background: newTitle.trim() ? "#185FA5" : "#ddd", color: newTitle.trim() ? "white" : "#999", border: "none", borderRadius: 10, padding: "10px 16px", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
          新增
        </button>
      </div>
